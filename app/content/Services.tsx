  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto w-[92%] max-w-7xl">
        ...
        {/* HEADER SECTION */}
        <FadeUp>
          <div className="mx-auto max-w-3xl text-center mb-10 sm:mb-16 lg:mb-20">
            {/* Live Indicator Pill */}
        ...
        {/* ========================================================= */}
        {/* MOBILE & TABLET VIEW: MINIMALIST BADGE / CHIP LIST        */}
        {/* ========================================================= */}
          ...
          {services.map((service, index) => {
            ...
            return (
              <FadeUp key={service.title} delay={index * 0.05}>
                <Link
                  href={service.href}
                  className="group flex items-center justify-between p-3.5 rounded-2xl border border-pink-200/70 bg-white shadow-[0_4px_15px_rgba(45,36,51,0.02)] active:scale-[0.98] transition-all duration-300 hover:border-pink-400 hover:bg-pink-50/30"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    {/* Badge Icon */}
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-pink-50 text-pink-500 border border-pink-200/80 group-hover:bg-pink-500 group-hover:text-white transition-colors">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        <h3 className="text-xs font-bold text-[#2D2433] truncate group-hover:text-pink-600 transition-colors">
                          {service.title}
                        </h3>
                      </div>
                      <p className="text-[10px] text-[#6B6570] truncate mt-0.5">
                        {service.category}
                      </p>
                    </div>
                  </div>
                  ...
                  {/* Arrow Icon */}
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-pink-50/80 text-[#2D2433] border border-pink-200/60 group-hover:bg-pink-500 group-hover:text-white transition-colors ml-2">
          ...
          {services.map((service, index) => {
            ...
            return (
              <FadeUp key={service.title} delay={index * 0.08} className="h-full">
                <Link
                  href={service.href}
                  className="group relative flex flex-col justify-between h-full overflow-hidden rounded-[32px] border border-pink-200/70 bg-white p-8 shadow-[0_10px_30px_rgba(45,36,51,0.03)] transition-all duration-500 ease-out hover:-translate-y-2 hover:border-pink-400/60 hover:bg-gradient-to-b hover:from-white hover:to-pink-50/20 hover:shadow-[0_24px_50px_rgba(233,106,152,0.14)]"
                >
                  ...
                  <div className="relative z-10 flex items-center justify-between">
                    <div className="flex items-center gap-3.5">
                      {/* Upgraded Icon Badge */}
                      <div className="flex h-13 w-13 items-center justify-center rounded-2xl bg-pink-50/80 text-pink-500 border border-pink-200/80 transition-all duration-500 group-hover:bg-pink-500 group-hover:text-white group-hover:border-pink-500 group-hover:scale-110 group-hover:shadow-md">
                        <IconComponent className="h-6 w-6 transition-transform duration-500 group-hover:rotate-6" />
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-mono font-bold text-pink-600 opacity-0 -translate-x-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0">
                        Explore
                      </span>
                      <div className="flex h-11 w-11 items-center justify-center rounded-full bg-pink-50/80 text-[#2D2433] border border-pink-200/70 transition-all duration-500 group-hover:bg-pink-500 group-hover:border-pink-500 group-hover:text-white group-hover:shadow-md">
                  ...
                  <div className="relative z-10 mt-8">
                    ...
                    <h3 className="mt-2 text-2xl font-black tracking-tight text-[#2D2433] transition-colors duration-300 group-hover:text-pink-600">
                      {service.title}
                    </h3>

                    <p className="mt-3 text-sm text-[#6B6570] leading-relaxed line-clamp-2">
                      {service.description}
                    </p>
                  </div>
